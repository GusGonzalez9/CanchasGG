...
// Functions
import { fetchMyCart, addCartToMyList } from '../../store/action-creators/carts'
...
class MyCartContainer extends React.Component {
  ...
  submitPurchase(purchaseId, orders){
    ...
    : submitCart(purchaseId, orders).then(res => {
        ...
        return (
          <div>
            <h1 className={globalStyle.h1}>
              My cart {myCart.purchase ? `#${myCart.purchase.id}` : 'has no products yet'}
            </h1>
            <div className={style.orders}>
              ...
              <button className={style.button} onClick={() => confirm('Submit order?') ? this.submitPurchase(myCart.purchase.id, myCart.orders) : null}>SUBMIT ORDER</button>
            </div>
          </div>
        )
    }
